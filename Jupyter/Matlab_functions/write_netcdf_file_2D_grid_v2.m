function [] = write_netcdf_file_2D_grid_v2(fname, colind, rowind,...
    ll_lons, ll_lats, data, pentad, ...
    start_time, end_time, overwrite, N_out_fields, ...
    ll_lon, ll_lat, d_lon, d_lat)        
...
if N_pentad ==1
    ...
    netcdf.putVar(ncid,varid_mm,data_out(3, :, :,:));
    netcdf.putVar(ncid,varid_mv,data_out(4, :, :,:));
    netcdf.putVar(ncid,varid_mi,nanmin(data_out(6, :, :,:),[],4)); % Min over all pentads, always only 2D
    netcdf.putVar(ncid,varid_ma,nanmax(data_out(7, :, :,:),[],4)); % Max over all pentads, always only 2D
    netcdf.putVar(ncid,varid_ndata,data_out(5, :, :,:));
end
